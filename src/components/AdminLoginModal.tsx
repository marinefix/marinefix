      ...
      <div className="bg-[#0b1329] border border-slate-700 rounded-2xl max-w-sm w-full p-6 space-y-4 shadow-2xl animate-fade-in text-slate-100">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            {error && <p className="text-[11px] text-rose-400 mt-1.5 font-medium">Invalid secret passcode.</p>}
          </div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="px-3.5 py-1.5 rounded-lg border border-slate-700 text-xs text-slate-300 hover:bg-slate-800 transition cursor-pointer"
            >
              Cancel
            </button>
            <button
              type="submit"
              className="px-4 py-1.5 rounded-lg bg-cyan-400 text-slate-950 text-xs font-bold hover:bg-cyan-300 transition cursor-pointer"
            >
              Unlock Mode
            </button>
          </div>
